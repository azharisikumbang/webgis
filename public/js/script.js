...
async function loadMap() {
  ...
  await request(apiUrl + "/geojson/geojson-v1.json").then(async (data) => {
    geojsonFeature = await data;
  });

  let countMahasiswaPerKecamatan;

  await request(apiUrl + "/api/wilayah").then(async (data) => {
    if (data.code == 200) {
        ...
        countMahasiswaPerKecamatan.map((kecamatan) => {
          if (
            geojsonFeature.features[i].properties.NAMOBJ.toUpperCase()
              .replace(" ", "")
              .replace(" ", "") ===
            kecamatan.kecamatan.toUpperCase().replace("_", "").replace("_", "")
          ) {
            geojsonFeature.features[i].properties = {
              ...geojsonFeature.features[i].properties,
              ...kecamatan,
            };
          }
...
async function mapClicked(e) {
  if (
    getCache("wilayah-" + e.target.feature.properties.kecamatan.toLowerCase())
  ) {
    return showOnPopup(
      getCache("wilayah-" + e.target.feature.properties.kecamatan.toLowerCase())
    );
  ...
  element.append(
    createPopupTitle(
      "Daftar Mahasiswa Wilayah " + e.target.feature.properties.kecamatan
    )
  );
  ...
  setCache(
    "wilayah-" + e.target.feature.properties.kecamatan.toLowerCase(),
    element
  );
...
function createMahasiswaTable(data) {
  ...
  data.map((element, index) => {
    ...
    detailLink.setAttribute(
      "onclick",
      "getMahasiswaDetail('" + element.nim + "')"
    );
...
async function getMahasiswaDetail(nim) {
  ...
  await request(apiUrl + "/api/mahasiswa/" + nim).then(async (res) => {
    if (res.code == 200) {
...
function createMahasiswaDetail(data) {
  ...
  for (const d in data) {
    ...
    tableBodyCell.innerHTML =
      d == "nim" ? d.toUpperCase() : d.charAt(0).toUpperCase() + d.slice(1);
